	...
	return Controller.extend("MANAGED_RECIPE.controller.reci_initiator", {
		...
		fnsendBack: function() {
			...
			oSendBackService.create("/Recipe_HeaderSet", payload, {
				success: function(oData) {
					if (oData.MsgType === "E") {
						ErrorHandler.showCustomSnackbar(oData.MsgLine, "Error"); // Changed by srikanth message to MsgLine
						busyDialog.close();
					} else {
						// Show snackbar
						that.fnClearallModel();
						ErrorHandler.showCustomSnackbar(oData.MsgLine, "success");  // Changed by srikanth message to MsgLine
